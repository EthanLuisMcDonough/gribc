...
pub fn run_block(
    ...
    if result.is_none() {
        println!("PREPARE POP: {}", block.allocations);
...
pub fn evaluate_lambda(
    ...
    let res = match body {
        LambdaBody::Block(block) => match run_block(block, this, runtime, program) {
            Some(ControlFlow::Return(value)) => LambdaResult {
                value,
                is_implicit: false,
            },
            _ => LambdaResult::imp(GribValue::Nil),
        },
        LambdaBody::ImplicitReturn(expr) => {
            LambdaResult::imp(evaluate_expression(&expr, this, runtime, program))
        }
    };
